...
local function handleInteraction(event)
    ...
    if event.type == "gift" then
        local count = math.max(1, tonumber(event.repeatCount) or 1)
        grantCoins(count)
        lastEventValue.Value = string.format("%s sent %s x%d", event.username or "TikTok User", event.giftName or "Gift", count)
        pushToast(nil, lastEventValue.Value, "gift")
    elseif event.type == "follow" then
        applySpeedBoost(5)
        lastEventValue.Value = string.format("%s followed the live", event.username or "TikTok User")
        pushToast(nil, lastEventValue.Value, "follow")
    elseif event.type == "like" then
        totalLikesValue.Value += math.max(1, tonumber(event.likeCount) or 1)
        lastEventValue.Value = string.format("%s sent likes (+%d)", event.username or "TikTok User", math.max(1, tonumber(event.likeCount) or 1))
